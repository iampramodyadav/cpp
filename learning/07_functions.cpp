...
std::vector<int> read_int_vector() {
    ...
    while (true) {
        ...
        if (std::cin.eof()) {
            ...
            break;
            //To break the while loop from the console using EOF,
            //you can type either Ctrl+D (on Unix-like systems) or
            //Ctrl+Z followed by Enter (on Windows).
            //This will signal the End-of-File to the program, and the loop will terminate as expected.
        }
